 ...
 * {@link #execute(Destination) executing} it. This one handles functions where the return type is a single primitive or
 * entity value.
 *
 * @param <ResultT>
 *            The type of the result entity, if any.
 */
public class SingleValueFunctionRequestBuilder<ResultT>
    extends
    FunctionRequestBuilder<SingleValueFunctionRequestBuilder<ResultT>, ResultT>
{
    @Nonnull
     ...
     */
    public SingleValueFunctionRequestBuilder(
    ...
    public ResultT execute( @Nonnull final Destination destination )
    {
        ...
        if( result instanceof VdmEntity ) {
            response.getVersionIdentifierFromHeader().peek(((VdmEntity<?>) result)::setVersionIdentifier);
        }
        return result;
    }
}
