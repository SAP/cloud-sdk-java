    ...
    private String getMetadataNamespace( @Nonnull final File metadataFile )
    {
        ...
        if( doc != null ) {
            ...
            if( schemas.getLength() > 1 ) {
                logger
                    .info(
                        "Expected to have a single schema tag in the metadata file {}, but found {} instead. The first schema with a defined namespace found will be used.",
    ...
    private String getMetadataServiceUrl( @Nonnull final File metadataFile )
    {
        ...
        if( doc != null ) {
            ...
            if( schemas.getLength() > 1 ) {
                logger
                    .info(
                        "Expected to have a single schema tag in the metadata file {}, but found {} instead. The first atom:link found will be used.",
                        metadataFile.getName(),
                        schemas.getLength());
            }
