...
class NavigationPropertyMethodsGenerator
{
            ...
            logger
                .warn(
                    "Unable to generate code for navigation property "
                        ...
                        + navigationProperty.getReturnEntityType().getName()
    ...
    private JFieldVar createNavigationPropertyConstant(
        final NavigationPropertyModel navigationProperty,
        final JDefinedClass associatedEntity,
        final boolean isOneToMany )
    {
        final String navPropertyConstantName = navigationProperty.getJavaConstantName();

        final JClass navPropertyType =
            isOneToMany
                ? codeModel.ref(NavigationProperty.Collection.class).narrow(entityClass, associatedEntity)
                : codeModel.ref(NavigationProperty.Single.class).narrow(entityClass, associatedEntity);
        ...
        final JFieldVar navPropertyConstantField =
            entityClass
                .field(JMod.FINAL | JMod.PUBLIC | JMod.STATIC, navPropertyType, navPropertyConstantName, initLink);
        navPropertyConstantField
            .javadoc()
            .add(
                String
                    .format(
                        "Use with available request builders to apply the <b>%s</b> navigation property to query operations.",
                        navigationProperty.getEdmName()));
