...
class ODataHeaderTest
{
    ...
    void testUpdateCustomHeader()
    {
        ...
        // Note: with Apache HttpClient 5 the CSRF token HEAD request is issued by the client-level
        // CsrfTokenInterceptor, which does not propagate the request's custom headers.
        verify(headRequestedFor(CSRF).withHeader("x-csrf-token", equalTo("fetch")));
