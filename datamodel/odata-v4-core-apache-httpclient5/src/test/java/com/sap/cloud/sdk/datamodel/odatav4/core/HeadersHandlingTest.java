...
class HeadersHandlingTest
{
    ...
    {
        // With Apache HttpClient 5 the CSRF token HEAD request is issued by the client-level
        // CsrfTokenInterceptor, which does not propagate the request's custom headers. The custom
        // headers are only sent on the actual (PATCH) request, so that is where we assert them.
        stubFor(head(anyUrl()).willReturn(ok().withHeader(X_CSRF_TOKEN_HEADER_KEY, X_CSRF_TOKEN_HEADER_VALUE)));
