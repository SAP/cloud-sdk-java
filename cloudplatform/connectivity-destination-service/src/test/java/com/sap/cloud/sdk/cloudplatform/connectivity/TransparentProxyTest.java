...
class TransparentProxyTest
{
    ...
    @Test
    void testNullPointerExceptionWhenIOExceptionOccurs()
        throws Exception
    {
        TransparentProxy.register("gateway", "tenant-id");
        ...
        Try<Destination> result = executeWithTenant(() -> loader.tryGetDestination("destination-with-io-error"));

        // When an IOException occurs, the response is null, which causes a NullPointerException
        // when trying to consume the entity in the finally block
        assertThat(result.isFailure()).isTrue();
        assertThat(result.getCause()).isInstanceOf(NullPointerException.class);
        assertThat(result.getCause().getMessage())
            .contains("Cannot invoke \"org.apache.http.HttpResponse.getEntity()\"");
    }
    ...
    // ========== Tests for DestinationOptions with various parameters ==========
