...
public class AuthTokenTenantResolvingTest
{
    ...
    @Deprecated
    private static class DummyLocalScopePrefixProvider
        implements
    ...
    public void before()
    {
        ...
        PrincipalAccessor
            .setPrincipalFacade(
                new com.sap.cloud.sdk.cloudplatform.security.principal.ScpCfPrincipalFacade(
                    new DummyLocalScopePrefixProvider()));
    ...
    @Test
    public void testWithProvidedJwt()
    {
        mockedAuthTokenFacade.executeWithAuthToken(new AuthToken(JWT.decode(AUTHORIZATION_TOKEN)), () -> {
            ...
            assertThat(mockedAuthTokenFacade.tryGetCurrentToken().get().getJwt().getClaim("zid").asString())
                .isEqualTo("my-tenant");
            assertThat(mockedAuthTokenFacade.tryGetCurrentToken().get().getJwt().getClaim("user_name").asString())
                .isEqualTo("John Doe");

            assertThat(TenantAccessor.getCurrentTenant().getTenantId()).isEqualTo("my-tenant");
            assertThat(PrincipalAccessor.getCurrentPrincipal().getPrincipalId()).isEqualTo("John Doe");
    ...
    {
        new AuthToken(JWT.decode(INVALID_TOKEN));
    }
}
